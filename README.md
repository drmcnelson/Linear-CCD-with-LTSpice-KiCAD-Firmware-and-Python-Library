...
The following LTSpice model based on the ADA4807 seems to meet our goals.  We use the first opamp in the package for the follower and then shift, invert and amplify the signal in the second stage following our sketch above for the configuration, gain setting and offset.  The green trace is the output from the emitter follower and the purple curve is the output from the second stage.  In our actual circuit we use a trim pot for the voltage applied to V+.
