...
Following are some examples of how the device has been used.  The first is a spectrometer, using G1200 grating (Thorlabs).  The spectrum is that of Fluorescent ceiling lights through a 200um fiber at a distance of about 8ft, with a 200um slit.  Notice the resolution and signal/noise are quite reasonable.  The input circuit helps preserve dynamic range, which as it turns out is one of the tricky parts in in recording these spectra (though it is also not so easy to appreciate at screen resolution).
