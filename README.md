...
The following LTSpice model based on the ADA4807 seems to meet our goals.  We use the first opamp in the package for the follower and then shift, invert and amplify the signal in the second stage following our sketch above for the configuration, gain setting and offset.  The green trace is the output from the sensor and the purple curve is the output from the second stage.  As can be seen the 2.5V to 1.9V singal from the sensor becomes a 0.1V to 3.1V for the ADC.  In our actual circuit we use a trim pot for the voltage applied to V+.
